...
'  -moz-box-shadow: 0 0.1em 0.1em rgba(255, 255, 255, 0.3) inset;' +// ==UserScript==
// @name         Show node ids
// @namespace    http://procentive.com
// @version      0.1
// @description  Displays form's node ids and attempts to auto fill forms with those ids
...
'    </aside>');
		...
		$(frameBody).find('#autofill').on('click', function(evt) {
			...
			$(frameBody).find('select').each(function() {
				var optionCount = $(this).find('option').length;
				var randomNumber = Math.floor(Math.random() * optionCount) + 1;
				var firstRandomOption = $(this).find('option:nth-child(' + randomNumber + ')');
				var firstRandomOptionValue = firstRandomOption.val();
				if (firstRandomOptionValue === "00") {
					$(this).find('option:nth-child(' + (randomNumber + 1)  + ')').attr('selected', 'selected');
				} else if (firstRandomOptionValue === "") {
					$(this).find('option:nth-child(' + (randomNumber - 1)  + ')').attr('selected', 'selected');
				} else {
					$(this).find('option:nth-child(' + (randomNumber)  + ')').attr('selected', 'selected');
				}
			});
			console.log($(frameBody).find('select option:last').val());
			$(frameBody).find('input[type="checkbox"][name!="auditready"]').trigger('click');
